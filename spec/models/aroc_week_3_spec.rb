...
# ----------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    #   end
    # end
    # grouped_orders.each_with_index do |order, id|
    #   items_for_user_3_third_order = order.items.map(&:name) if id == 2
    # end
    # ------------------------------------------------------------

    # ------------------ Using ActiveRecord ----------------------
    items_for_user_3_third_order = Order.select('orders.id')
                                        .joins(:items)
